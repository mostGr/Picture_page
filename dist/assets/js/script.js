...
var filter = function filter() {
  ...
  menu.addEventListener('click', function (e) {
    var target = e.target,
        clickBtn = target.className.split(' ')[0];

    if (clickBtn === 'grandmother' || clickBtn === 'granddad') {
      typeFilter();
    } else {
      typeFilter(wrapper.querySelectorAll('.' + clickBtn));
    }
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    ...
    this.value = matrix.replace(/./g, function (a) {
      //к каждому символу в matrix применяется функция 
...
var modals = function modals() {
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      ...
      if (!btnPresssed && window.pageYOffset + document.documentElement.clientHeight >= scrollHeight) {
...
var scrolling = function scrolling(upSelector) {
  ...
  }); // Скролинг на простом js
  // const element = document.documentElement,
  //     body = document.body;
  // const calcScroll = () => {
  //     upElem.addEventListener('click', function(event) {
  //         let scrollTop = Math.round(body.scrollTop || element.scrollTop); // что из этого будет, то и попадет в переменную, иначе баг может быть
  //         if(this.hash !== '') {
  //             event.preventDefault();
  //             let hashElement = document.querySelector(this.hash),
  //                 hashElementTop = 0; // сколько px до родителя this.hash элемента
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      ...
      card.innerHTML = "\n                <div class=\"styles-block\">\n                    <img src=".concat(src, " alt = \"\u041F\u0440\u0438\u043C\u0435\u0440 \u0440\u0430\u0431\u043E\u0442\u044B\">\n                    <h4>").concat(title, "</h4>\n                    <a href=").concat(link, ">\u041F\u043E\u0434\u0440\u043E\u0431\u043D\u0435\u0435</a>\n                </div>\n            ");
  ...
  function activateAnimation() {
    if (dir === 'vertical') {
      paused = setInterval(function () {
        plusSlides(1);
        items[sliderIndex - 1].classList.add('fadeIn');
      }, 3000);
    } else {
      paused = setInterval(function () {
        plusSlides(1);
